...
class TerminalFlightMixDataset(Dataset):
    def __init__(self, input_dir, n_his, n_pred, is_continous, dates_dist, interval= 5, normalizer= "std"):
        ...
        # self.terminal_max = np.max(features.numpy())
    ...
    def generate_data(self):
        ...
        flight_count_all = Tensor(np.array(flight_count_all))
        timestamps_all = timestamps_all

        return his_count_all, target_count_all, flight_count_all, timestamps_all
    ...
    def mix_data(self):
        ...
        """
        his_count_all, target_count_all, flight_count_all, timestamps_all = self.generate_data()
        ...
        extra_xs = flight_count_all.unsqueeze(1) 
        extra_xs = extra_xs.unsqueeze(2) # extra_xs: (total_len, 1, 1, n_pred)
        extra_xs = extra_xs.repeat(1, xs.shape[1], xs.shape[2], 1) # extra_xs: (total_len, n_his, num_nodes, n_pred)

        features = torch.concat((xs, extra_xs),-1) # features :(total_len, n_his, num_nodes, in_dim = n_pred+ 1)
...
if __name__=="__main__":
    ...
    tfdata = TerminalFlightMixDataset(input_dir="data/temp", n_his=6, n_pred=3, is_continous=False, dates_dist=[["20240609", "20240610"],["20240612", "20240613"]], interval = 5)
