...
class MetricTracker:
    def __init__(self, *keys, writer=None, is_epoch = False):
        self.writer = writer
        self._data = pd.DataFrame(index=keys, columns=['total', 'counts', 'average'])
        self._data_epoch = pd.DataFrame(index = keys, columns=['kvalue'])
    ...
    def reset(self):
        if not self.is_epoch:
            ...
            for col in self._data_epoch.columns:
                self._data_epoch[col].values[:]=0
    ...
    def update(self, key, value, n=1):
        if not self.is_epoch:
            ...
            self._data_epoch.kvalue[key] += value
    ...
    def acc(self):
        assert self.is_epoch, "Accumulate value should be used in epoch scenario!"
        return dict(self._data_epoch.kvalue)    
