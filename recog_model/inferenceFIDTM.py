...
import sys
sys.path.append('.')
from recog_model.FIDTM.seg_hrnet import get_seg_model
...
def covert_img(img_path, device):
...
@torch.no_grad()
def predict(model, img_path, device="cuda"):
    ori_img, img = covert_img(img_path, device)
    
    h, w = img.shape[2:]
    output = model(img)
    pred_count, pred_kpoint= counting(output)
    pred_dmap= generate_point_map(pred_kpoint)

    return ori_img, pred_dmap, pred_count
...
if __name__=="__main__":
    ...
    config_model = config["crowd_model"][model_name]
    ...
    model = load_model(config_model["pretrained"], config_model["device"])
    ...
    for area in areas:
        ...
        for date in dates:
            ...
            for frame_name in frame_names:
                ...
                img, pred_dmap, pred_count =predict(model, frame_path, config_model["device"])
                if(pred_count > 0):
                    print(f"there is {pred_count} in {frame_name}")
                confidence = None
                ...
                if vis_dir is not None:
                    ...
                    pred_dmap = cv2.cvtColor(pred_dmap, cv2.COLOR_BGR2RGB)
                    ax_dmap.imshow(pred_dmap)
                    
                    ax_dmap.set_title("Heatmap",y=-0.2)
                    area, date, _ = frame_name.split("_")
                    # 为整个大图设置标题
                    fig.suptitle(f"Device: {area}   Date: {date}   Time: {time_str}  Count: {pred_count}", fontsize=16, y=0.75)
